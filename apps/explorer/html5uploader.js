	...
	upload = function(file) {
		...
		if(window.FileReader) { 
			...
			this.loadEnd = function() {
				...
				if (status) {
					document.getElementById(status).innerHTML = 'Saved! Drop your files here';
				}
			}
			...
			this.loadProgress = function(event) {
				if (event.lengthComputable) {
					...
					document.getElementById(status).innerHTML = 'Saved : '+percentage+'%';
				}				
			}
    	...
    	if (show) {
  		...
  		// Safari 5 does not support FileReader
			...
			if (status) {
				document.getElementById(status).innerHTML = 'Saved : 100%';
			}
